...
- **人类面向文件（全中文）**：最终交付给开发者阅读的产物（如通过 `project-analyzer` 生成的《项目白皮书》），被严格限制为使用纯中文输出，并要求符合专业的技术文档排版规范。
...
这些技能均采用了多智能体协作的模式。在 `code-reader` 中，通过 `QA` 工程师生成测试题并由初级开发者仅根据文档进行解答的验证循环，有效避免了浅尝辄止的代码总结。在 `project-analyzer` 中，则通过首席架构师汇总代码逻辑和基础设施配置，确保最终输出的架构图和文档在逻辑上保持一致且专业严谨。
