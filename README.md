...
`md-summarizer` 技能用于分析和总结本地的 Markdown 文件，并输出结构化、专业深度的中文分析报告。
...
`web-content-downloader` 技能用于下载指定的网页内容，剥离无关的 HTML 标签并转换为标准的 Markdown 格式，同时保留网页原始语言（不进行翻译）。
...
`md-translator` 技能用于将指定的本地 Markdown 文件翻译成目标语言（默认中文），并在生成的新文件中添加语言标识后缀。
...
`reference-organizer` 技能用于自动获取外部链接（如博客、技术白皮书、学术论文等）的元数据，并根据用户要求的级别和场景，将其格式化为标准的参考文献条目。
...
`md-link-checker` 技能用于自动化验证 Markdown 文件中的本地和外部链接有效性，确保项目文档内链与外链的连通性。
...
从生产级目录组织到渐进式上下文加载，一套标准化的工程规范是确保智能体技能稳定运行的基石。以下实践均参考自 [给 Claude 写本“标准操作手册”：Agent Skills 实战与深度解析](https://github.com/ForceInjection/AI-fundermentals/blob/main/08_agentic_system/agent-skills/Claude_Skills_Guide.md) 文档。
