...
To address engineering challenges such as unfamiliar codebases, project reverse engineering, and spec-driven development, this project packages **19 standalone agent skills** designed to solve real development bottlenecks through multi-role collaboration.
...
| [`humanizer-zh`](./skills/humanizer-zh)                       | Chinese AI-slop remover: detects and fixes 35 AI writing tells (inflated claims, sales language, stock AI words, dash abuse, rule-of-three structures, etc.), rewriting into natural human prose without changing meaning; based on Wikipedia's "Signs of AI writing" | `/humanizer-zh <text or file>`                |
...
| [`pptx-editor`](./skills/pptx-editor)                         | PPTX editor: locate shapes by semantic name for run-level text replacement and styled element insertion, verified by LibreOffice + pdftotext rendering (no overflow, no collisions)                                                                                   | `/pptx-editor <target-file>`                  |
...
> Provenance: `ontology` is imported from [hanzoskill/ontology](https://github.com/hanzoskill/ontology) (locally enhanced superset), `editorial-card-designer` is imported from [shaom/infocard-skills](https://github.com/shaom/infocard-skills) (renamed & hardened locally), `pptx-reader` is based on [anthropics/skills](https://github.com/anthropics/skills/blob/main/skills/pptx/SKILL.md), and `humanizer-zh` is translated from [blader/humanizer](https://github.com/blader/humanizer) (adapted for Chinese prose). Full usage examples and end-to-end demos live in each skill's `SKILL.md` and the `examples/` directory.
...
### 4.4 mattpocock/skills Deep Dive

A deep analysis of Matt Pocock's (Total TypeScript founder) 220k+-star production skill library: a **composition-over-content** architecture (primitives + thin routers), the grilling design-tree interviews, the wayfinder decision-ticket maps, seam-governed TDD, the diagnosing-bugs loop discipline, and the writing-for-agents meta-skill — compared one-to-one with this repo's skill system. Read the full report: [mattpocock/skills deep-dive](./docs/mattpocock-skills-deep-dive-en.md).
...
| [mattpocock/skills](https://github.com/mattpocock/skills)                                                   | Engineering discipline       | Matt Pocock's (Total TypeScript founder) daily-driver skills: grilling design-tree interviews, wayfinder decision-ticket maps, seam-governed TDD — 35 composable skills via the Claude Code marketplace |
