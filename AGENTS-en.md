...
- **PPTX Editor (`pptx-editor`)**: Makes precise, verifiable edits to existing `.pptx` files — locating shapes by semantic name for run-level text replacement, inserting elements styled after neighbors, and verifying no overflow or collisions via LibreOffice rendering + pdftotext word coordinates.
