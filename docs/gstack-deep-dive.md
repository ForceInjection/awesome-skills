...
- [5. 典型技能拆解与 Prompt 工程最佳实践](#5-典型技能拆解与-prompt-工程最佳实践)
  ...
  - [5.2 Prompt 工程最佳实践总结](#52-prompt-工程最佳实践总结)
    - [5.2.1 结构化输入解析与防御性设计](#521-结构化输入解析与防御性设计)
    - [5.2.2 跨阶段上下文继承](#522-跨阶段上下文继承)
    - [5.2.3 注入专家级“思维模式”](#523-注入专家级思维模式)
    - [5.2.4 动态编排与人机交互](#524-动态编排与人机交互)
...
- **角色化工作流（23 个核心技能）**：提供从产品规划（找寻 10 星级产品）、质量保障（寻找深层逻辑漏洞）、发布运营到基础设施的完整生命周期覆盖。
...
gstack 的系统架构主要分为两个维度：**AI 技能调度层**和**无头浏览器交互层**。为了解决 AI 代理在频繁调用浏览器时遇到的冷启动延迟（约 2-3 秒）和状态丢失（如 Cookie、登录态）问题，gstack 创新性地引入了 C/S 架构的守护进程模型。
...
1. **Bun 的极致应用**：
   ...
   - 使用内置的 `Bun.serve()` 提供极简的 HTTP 服务，处理 20+ 个核心路由，避免了冗余的 Express/Fastify 框架开销。
...
5. **日志架构**：
   - 采用三个环形缓冲区（Ring Buffer），每个容量为 50,000 条记录，分别存储 Console、Network 和 Dialog 事件。
...
- **HTTP 守护服务 (`server.ts`)**
  ...
  - **逻辑**：实现页面的读取与写入接口，内置空闲超时策略（默认 30 分钟无操作则释放资源）。采用环形缓冲区（Ring Buffer）机制，将浏览器控制台日志、网络请求日志及弹窗事件异步持久化至本地磁盘，以降低 I/O 阻塞。
...
- **阶段化执行与闭环**：
  - 从 `Phases 1-6: QA Baseline`（`Phase 1: Initialize` 至 `Phase 6: Wrap Up`）到 `Phase 7: Triage`（基于设定的 Tier 过滤 Bug），再到 `Phase 8: Fix Loop`、`Phase 9: Final QA` 与 `Phase 10: Report`。
...
- **认知模式 (Cognitive Patterns) 硬编码**：
  - Prompt 中直接写入了十余条“优秀工程经理如何思考”的准则。例如：“爆炸半径直觉”、“默认选择无聊技术（Boring by default）”、“两周异味测试”以及“系统优于个人”。这赋予了 AI 评判架构设计好坏的“价值观”。
...
在这个流程中，得益于守护进程架构，只有第一次调用会触发冷启动，后续的 `HTTP POST` 交互将延迟压缩在了 100-200 毫秒内。

---

## 7. 质量与性能评估 (含 AI 技能测试方法论)

本章节从系统性能表现、自动化测试覆盖率等多个维度对项目的工程质量进行综合评估，并提炼出测试 AI Skill 的最佳实践。

### 7.1 系统性能表现

得益于常驻内存的守护进程架构，浏览器的首次启动时间约为 2-3 秒，但后续所有的 DOM 交互、快照截取和网络请求等命令的延迟均被压缩至 **100-200 毫秒** 之间。这使得 AI 代理可以像人类一样流畅地“浏览”页面，极大地提升了 `/qa` 和 `/design-review` 技能的执行效率。
...
- **内容**：通过 `test/helpers/llm-judge.ts`，裁判模型会读取并评估 AI 代理生成的 QA 报告或设计文档：
...
- **内置打包能力**：利用 `bun build --compile` 直接将 TypeScript 源码（如无头浏览器 CLI）编译为体积约 58MB 的单文件二进制可执行程序。这意味着最终用户在运行 gstack 时，甚至不需要在机器上安装 Node.js 环境（macOS/Linux 下）。
...
为了让 AI 助手在任何项目中都能调用 gstack 的 23 个核心技能，推荐将其安装到全局的 `~/.claude/skills` 目录中。
...
> 本文基于 2026-07 的 gstack 仓库快照撰写，仓库持续快速演进，部分数据可能过时。
