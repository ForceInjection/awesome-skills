...
[superpowers](https://github.com/obra/superpowers) 是为 AI 编码智能体（如 `Claude Code`、`Cursor`、`Codex` 等）提供完整软件开发工作流的插件和技能（`Skill`）集合。项目通过引入“子智能体驱动开发（`SDD`）”模式，强制 AI 在编写代码前进行需求澄清与任务拆解。核心特性包括：**测试驱动开发（`TDD`）**的强制执行、**一体化任务审查**（规范符合性与代码质量在同一次审查中完成），以及通过**隔离的 Git 工作树**和**独立子智能体**避免长上下文污染。该设计将 AI 从单纯的代码生成器转变为遵循严谨工程规范的虚拟开发团队。
...
`SKILL.md` 是项目的核心执行契约，由高度结构化的“人设与工作流约束”构成。以下提取 [`dispatching-parallel-agents/SKILL.md`](https://github.com/obra/superpowers/blob/main/skills/dispatching-parallel-agents/SKILL.md) 的骨架作为示例：
...
通过对比上述示例与项目中的其他技能文件（如 [`systematic-debugging/SKILL.md`](https://github.com/obra/superpowers/blob/main/skills/systematic-debugging/SKILL.md)），我们可以归纳出编写一个标准技能（`Skill`）所必须遵循的结构范式：
...
项目以提示词模板形式定义审查智能体的人设与审查维度（[Code Reviewer Prompt Template](https://github.com/obra/superpowers/blob/main/skills/requesting-code-review/code-reviewer.md)，位于 `skills/requesting-code-review/` 目录），确保代码生成与代码审查的关注点隔离。审查员被设定为高级代码审查员（Senior Code Reviewer），通过明确的 Prompt 约定其核心审查维度：
...
- **分级反馈与沟通协议**：审查意见必须严格按照“Critical (Must Fix) / Important (Should Fix) / Minor (Nice to Have)”三级分类输出；若发现严重偏离计划，必须要求代码生成智能体进行确认和修改。

### 3.3 钩子与命令模块

平台集成依赖于钩子拦截和快捷命令。

- **全局上下文注入**：[`hooks/session-start`](https://github.com/obra/superpowers/blob/main/hooks/session-start) 脚本在会话初始化时被触发。它通过 Bash 脚本读取 [`skills/using-superpowers/SKILL.md`](https://github.com/obra/superpowers/blob/main/skills/using-superpowers/SKILL.md) 的内容，并使用特定格式（如 Cursor 的 `additional_context` 或 Claude Code 的 `hookSpecificOutput`）将其注入为全局 `<EXTREMELY_IMPORTANT>` 上下文，从而让智能体在对话伊始就“知道”自己拥有调用技能的能力。
- **规范化引导指令**：头脑风暴以技能形式存在（[`skills/brainstorming/SKILL.md`](https://github.com/obra/superpowers/blob/main/skills/brainstorming/SKILL.md)），要求 AI 先了解项目上下文，再一次只问一个问题，通过多轮澄清精炼用户需求，强制其在生成任何代码或计划前，先消除所有的歧义与假设。
...
针对计划中的每个任务，主智能体派发一个全新的实现者子智能体（隔离上下文）。该子智能体必须遵循 `test-driven-development` 技能，执行“红-绿-重构”（Red-Green-Refactor）循环：先编写失败的测试用例，再编写极简代码使其通过，最后进行提交和自我审查。

### 4.3 质量审查与代码合并

实现者提交代码后，系统为每个任务派发一个 task reviewer（提示词模板见 `skills/subagent-driven-development/task-reviewer-prompt.md`），由它读取任务 diff，在同一轮审查中同时给出规范符合性（spec compliance）与代码质量（code quality）两项结论：既核对实现是否满足原计划需求、无遗漏或过度设计，也评估代码是否干净、可测试、可维护。发现问题即打回要求修复。全部任务完成后，还有一次针对整个分支的复审（提交总结并核对最终 diff），随后触发 `finishing-a-development-branch` 技能进行最终测试验证与分支合并。
...
sequenceDiagram
    ...
    loop 每个子任务
        ...
        Orchestrator->>Reviewer: 派发 task reviewer（规范+质量一次完成）
        Reviewer-->>Orchestrator: 审查结果（spec compliance + code quality）

        opt 存在问题
            Orchestrator->>Subagent: 修复偏差或代码异味
        end
    end

    Orchestrator->>Reviewer: 整分支复审（提交总结 + 最终 diff 核对）
...
技能文档的开发遵循“红-绿-重构”循环，以测试驱动的方式编写和验证，确保智能体准确执行技能约束。编写技能前，必须先编写失败的测试用例。

具体流程如下：

- **红-绿-重构（红灯阶段）**：设计包含多种压力的测试场景，在不加载目标技能的情况下运行智能体，记录其违背规范的基线行为。
- **红-绿-重构（绿灯阶段）**：编写极简的技能文档，明确核心原则和适用场景，再次运行测试，验证智能体在技能的约束下是否能够规范执行。
- **红-绿-重构（重构阶段）**：分析测试中出现的新的合理化借口，进一步完善技能文档以堵住漏洞。
...
> - 启用本地 dev marketplace：在 `~/.claude/settings.json` 中启用本地 marketplace（示例配置：`"superpowers@superpowers-dev": true`，实际写法以插件安装文档为准）。
...
3. 这个技能是作为独立的验证步骤，还是集成到现有的 `requesting-code-review` 流程中？
...
- **Task 1**：在 `tests/` 下编写测试脚本 `test-md-linter.sh`，准备一个包含格式错误的测试用例文档，并断言 AI 能否正确发现问题（体现了“红-绿-重构”理念）。
...
用户要求执行计划（触发 `subagent-driven-development` 技能）。
...
- **审查**：主智能体派发 **task reviewer**，在同一轮审查中确认测试用例涵盖了“中英文空格缺失”的场景并检查代码质量。
...
superpowers 面向 AI 编码智能体，提供规范化的软件开发工作流。它以声明式 `SKILL.md` 为核心，通过会话钩子注入上下文，并以审查/实现子智能体编排流程，将开发活动产品化。在工程机制上，它强制 TDD 与规范/质量一体化的任务审查，隔离子智能体上下文并支持并行调度，结合隔离的 Git 工作树与基于真实交互的技能验证，确保安全、可验证的迭代。其落地路径为“写计划 → 在隔离工作树按计划执行 → 运行技能与集成测试验证 → 审查通过后收尾合并”。
