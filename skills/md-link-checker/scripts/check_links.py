...
def check_local_file_exists(file_path, base_dir):
    ...
    if not exists:
        # 启发式探测：如果某个父目录存在且为空，极可能是未初始化的 Submodule
        current_check = full_path
        while current_check and current_check != base_dir and current_check != '/':
...
def main():
    ...
    # 自动获取项目根目录
    # 脚本路径：<SKILL_DIR>/scripts/check_links.py
    ...
    for file_path in md_files:
        ...
        if args.type in ['external', 'all']:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                future_to_link = {executor.submit(check_external_url, url): (text, url) for text, url in external_links}
                for future in concurrent.futures.as_completed(future_to_link):
                    ...
                    try:
                        ...
                        external_stats['invalid'].append((file_path, text, url, str(e)))
