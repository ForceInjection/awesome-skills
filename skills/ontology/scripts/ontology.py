...
try:
    ...
    yaml = None  # Schema operations will raise a clear error if PyYAML is missing
...
def _require_yaml():
    ...
    if yaml is None:
        raise SystemExit(
            "PyYAML is required for schema operations. Install it with: pip install pyyaml"
        )


def validate_entity_against_schema(type_name: str, properties: dict, schema: dict) -> list[str]:
    """Validate a single entity's properties against a schema. Returns a list of error strings."""
    errors = []
    ...
    for prop in type_schema.get("forbidden_properties", []):
        if prop in properties:
            errors.append(f"contains forbidden property '{prop}'")

    # Check enum values
    for key, allowed in type_schema.items():
        if key.endswith("_enum"):
            field = key.replace("_enum", "")
...
def create_entity(type_name: str, properties: dict, graph_path: str, entity_id: str = None, schema_path: str = None) -> dict:
    """Create a new entity. Validates against schema if schema_path is provided and the schema file exists."""
    if schema_path:
        schema = load_schema(schema_path)
        if schema:
            errors = validate_entity_against_schema(type_name, properties, schema)
            if errors:
                raise SystemExit(f"Validation failed for {type_name}: {'; '.join(errors)}")
...
def create_relation(from_id: str, rel_type: str, to_id: str, properties: dict, graph_path: str):
    """Create a relation between entities. Verifies both entities exist."""
    entities, _ = load_graph(graph_path)
    if from_id not in entities:
        raise SystemExit(f"Entity not found: {from_id}")
    if to_id not in entities:
        raise SystemExit(f"Entity not found: {to_id}")
...
def validate_graph(graph_path: str, schema_path: str) -> list:
    ...
    for rel_type, rel_schema in relation_schemas.items():
        ...
        # Acyclic checks using three-color DFS (WHITE=0, GRAY=1, BLACK=2)
        if acyclic:
            adj = {}
            for rel in rels:
            ...
            def has_cycle(node):
                ...
                for nxt in adj.get(node, []):
                    if color.get(nxt, WHITE) == GRAY:
                        return True
                    if color.get(nxt, WHITE) == WHITE and has_cycle(nxt):
                        return True
                color[node] = BLACK
                return False
...
def main():
    ...
    create_p.add_argument("--schema", "-s", default=DEFAULT_SCHEMA_PATH)
    ...
    if args.command == "create":
        ...
        entity = create_entity(args.type, props, args.graph, args.id, schema_path=args.schema)
