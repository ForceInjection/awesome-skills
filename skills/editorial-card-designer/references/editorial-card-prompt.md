...
- 关键说明区可使用浅色强调底块 `rgba(0,131,203,0.06)` 或 EMC Blue 淡底。
...
- `16:9` / `9:16`：
  - 适合标准宽屏封面或竖屏故事版。
  - 横向展开，纵向空间有限，应大幅精简段落长度。
  - 用更少但更大的模块、更强的标题、更短的支持文案。
- `2.35:1` / `3:1` / `5:2`（超宽封面）：
  - 大幅度减少段落长度。
  - 使用更少但更大的块、更强的标题、极短的支持行。
  - `3:1` 需特别注意：保持文本稀疏，保护可能的重叠区域（如头像区），避免满宽铺字。
...
- 用户提供的标题被擅自改写，即使当前比例可以通过更好的布局容纳原标题。
- 标题与正文/摘要发生视觉重叠、碰撞或遮挡。
- 标题在横向宽度充足时被挤成细长的竖条多行。
- 高密度卡片被切成过多等权重的盒子。
- 大模块内文字过少，读起来像空容器。
- 画布出现大量可通过加强层次、丰富内容或加重主模块来填充的死区。
- PNG 比 HTML 布局意图明显更空。
- PNG 因远程字体失败且未提供本地 fallback 字体栈，导致阅读节奏与 HTML 不一致。
- Footer 或底部条带被裁掉，或因网格尺寸把内容推离下边缘而出现大块底部空白。
